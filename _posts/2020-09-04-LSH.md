...
### imilarity Search in High Dimensional Space
...
We will use the digit data from sklearn.datasets. Here we plot the first 10 data points, each data point represents a 64 number array with each number giving an amount of black colour.
...
We now select our query point and prepare the data for our c2lsh function.

```python
data = [np.concatenate(digits.images[j], 0).tolist() for j in range(1000)]
query = [np.concatenate(digits.images[0], 0).tolist()][0]
...
We can now run our c2lsh on the digit data to find the closest 50 points to our query.
...
#Run C2LSH
start_time = time()
res = c2lsh(data_hashes, query, alpha_m, beta_n).collect()
end_time = time()
sc.stop()
...
```python
running time: 4.026445627212524
Number of candidate:  52
Key of candidate:  {770, 144, 274, 19, 404, 405, 535, 664, 920, 665, 281, 151, 541, 25, 671, 168, 42, 428, 558, 688, 564, 565, 694, 695, 951, 187, 317, 447, 833, 577, 963, 323, 453, 193, 583, 969, 74, 463, 979, 723, 213, 343, 603, 859, 733, 223, 483, 357, 999, 487, 747, 122}
```
As can be seen we have succesfully retrieved the 50 most similar points!
```python
#Plot our candidates
...
for i in range(len(res)):
    ax = fig.add_subplot(8, 8, i + 1, xticks=[], yticks=[])
    ax.imshow(digits.images[len(data) - res[i] - 1], cmap=plt.cm.binary, interpolation='nearest')
    ax.text(0, 7, str(digits.target[len(data) - res[i] - 1]))
```
![](/images/lsh7.png)
